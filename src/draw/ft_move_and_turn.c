...
/*   Updated: 2023/03/27 23:50:57 by ryoshio-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (cub->play.move == UP && 
	ft_iswall(cub->data.map, cub->play.y -  2 * sin(cub->play.ang), cub->play.x + 2 * cos(cub->play.ang) )  == SUCCESS) // move para esquerda
	{
...
static void ft_move_utils(t_cub *cub)
{
	...
	ft_iswall(cub->data.map, cub->play.y +  2 * cos(cub->play.ang), cub->play.x + 2 * sin(cub->play.ang) )  == SUCCESS) // move para esquerda
	{
