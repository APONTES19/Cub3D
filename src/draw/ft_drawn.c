...
void ft_drawn_wall(t_cub *cub, t_img  *image,  int i)
{
	...
	if(cub->play.status == HORIZONTAL)
		offset_x = (int)floor( cub->play.x + cub->play.dist * cos(cub->play.ray_ang)) % TEXTURE_SIZE;
	else
	    offset_x = (int) floor( cub->play.y - cub->play.dist * sin(cub->play.ray_ang)) % TEXTURE_SIZE;
	...
	if( size == 0)
		...
		offset_y = ((image->bits_per_pixel)* (i - (WIN_SIZE_Y - size)/2))/size ;
