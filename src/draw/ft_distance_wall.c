...
/*   Updated: 2023/03/30 05:28:25 by ryoshio-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/cub3d.h"

static double	ft_loop_distance(t_cub *cub);
static double	ft_horizontal_wall(t_cub *data);
static double	ft_vertical_wall(t_cub *data);
...
void	ft_distance_wall(t_cub *data)
{
	...
	correction = cos(ft_radian_domain(data->play.ang - data->play.ray_ang));
...
static double	ft_horizontal_wall(t_cub *data)
{
	...
	if (sin(data->play.ray_ang) > 0)
	{
		...
		data->play.yo = floor(data->play.y / TEXTURE_SIZE)
			* TEXTURE_SIZE - 0.001;
		data->play.xo = (data->play.y - data->play.yo)
			* cos(data->play.ray_ang) / sin(data->play.ray_ang) + data->play.x;
		data->play.dx = TEXTURE_SIZE
			* cos(data->play.ray_ang) / sin(data->play.ray_ang);
	}
	else
	{
...
static double	ft_vertical_wall(t_cub *data)
{
	...
	if (cos(data->play.ray_ang) > 0)
	{
		...
		data->play.xo = floor(data->play.x / TEXTURE_SIZE)
			* TEXTURE_SIZE + TEXTURE_SIZE;
		data->play.yo = tan (data->play.ray_ang)
			* (data->play.x - data->play.xo) + data->play.y;
	}
	else
	{
		data->play.dx = -1 * TEXTURE_SIZE;
		...
		data->play.xo = floor(data->play.x / TEXTURE_SIZE)
			* TEXTURE_SIZE - 0.001;
		data->play.yo = tan (data->play.ray_ang) *(data->play.x - data->play.xo)
			+ data->play.y;
	}
	return (ft_loop_distance(data));
}
