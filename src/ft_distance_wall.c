...
/*   Updated: 2023/03/12 22:33:46 by ryoshio-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static float ft_horizontal_wall (t_cub *data)
{
	if(sin(data->play.ang) > 0.001593) // olhando para cima
    {
		data->play.dy = -1* TEXTURE_SIZE;
		data->play.yo  = floor( data->play.y /TEXTURE_SIZE) * TEXTURE_SIZE - 0.00001 ;
		data->play.xo = (data->play.y -data->play.yo)/tan (data->play.ang) +  data->play.x;
    	data->play. dx = -1*data->play.dy/tan(data->play.ang);
    } 
	else if(sin(data->play.ang) < - 0.001593) // olhando para baixo
	{
	...
	printf("\n angulo =%f",  sin(data->play.ang));
	printf("\n xo =%f, yo= %f", data->play.xo, data->play.yo );
...
static float ft_vertical_wall (t_cub *data)
{
	...
	if(cos(data->play.ang) > 0.001593) // direito
    {
		data->play.dx = TEXTURE_SIZE;
		data->play.dy = 1*tan (data->play.ang) * data->play.dx;
		data->play.xo = ceil(data->play.x /TEXTURE_SIZE) *TEXTURE_SIZE+ 0.00001;
		data->play.yo  = 1* tan (data->play.ang) *(data->play.xo - data->play.x) + data->play.y;

    	
    } 
	else if(cos(data->play.ang) < - 0.001593) // esquerdo
	{
		data->play.dx = -1 * TEXTURE_SIZE;
		data->play.dy = -1* tan (data->play.ang) * data->play.dx;
			data->play.xo = floor( data->play.x/TEXTURE_SIZE) *TEXTURE_SIZE - 0.00001 ;
		data->play.yo =  tan (data->play.ang) *(data->play.xo  - data->play.x) + data->play.y;
